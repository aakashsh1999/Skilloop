...
const MessagesListScreen = () => {
  ...
  const loggedInUserId = typeof session === 'string' ? session : (session as any)?.userId;
  ...
  useEffect(() => {
    if (!loggedInUserId) {
      console.warn("MessagesListScreen: No loggedInUserId found for socket connection.");
      return;
    }
    ...
    socket.on("connect_error", (err) => {
      console.error("MessagesListScreen: Socket connection error:", err.message);
      Alert.alert(
        "Connection Error",
